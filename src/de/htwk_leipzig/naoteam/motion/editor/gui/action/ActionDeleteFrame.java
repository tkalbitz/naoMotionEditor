...
public class ActionDeleteFrame extends AbstractAction  {

    /** controller to manipulate frames */
    protected MainApplication app;

    @Inject
    ActionDeleteFrame(IImageLoader loader, MainApplication appController) {
        super("Delete Frame");
        final String name = "list-remove.png";
        putValue(Action.LARGE_ICON_KEY, loader.large(name));
        putValue(Action.SMALL_ICON, loader.small(name));
        putValue(SHORT_DESCRIPTION, "Delete current Frame");
        app = appController;
    }

    public void actionPerformed(ActionEvent e) {
    	app.createUndoStep();
    	app.getFrameList().remove(app.getCurrentSelectedFrameIndex());
    }
}
