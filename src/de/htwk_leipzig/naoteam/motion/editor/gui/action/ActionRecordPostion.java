...
public class ActionRecordPostion extends AbstractAction  {
    ...
    @Inject
    ActionRecordPostion(IImageLoader loader, MainApplication appController) {
        super("Record Position");
        final String name = "mix_record.png";
        putValue(Action.LARGE_ICON_KEY, loader.large(name));
        putValue(Action.SMALL_ICON, loader.small(name));
        app = appController;
        putValue(SHORT_DESCRIPTION, "Record the current position of the robot.");
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        final NaoFrame frame = new NaoFrame(newPos, bodyParts);
        app.createUndoStep();
        app.getFrameList().add(app.getCurrentSelectedFrameIndex() + 1, frame);
    }
